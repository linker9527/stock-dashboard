...
**9. 东财 push2 实时接口有 IP 级封禁（2026-09-24 实测）**
- 触发：短时间内多股票高频轮询（数分钟约 40 个请求的实测量级即触发）
- 症状：**TLS 握手成功、HTTP 请求发出后被服务端直接 RST**（curl 报 `code 56: server closed abruptly (missing close_notify)`），无任何 HTTP 响应，持续 25 分钟以上不解封
- 范围：`push2.eastmoney.com` 及编号子域（1.push2/82.push2）全封；但 **`push2his`（K线）、`searchapi`（搜索/补名）不受影响**——所以封禁期间行情降级到新浪/腾讯，K线和中文名仍是东财
- 对策：降级链已验证有效；3 秒缓存只按 code 去重，多 code 峰值防不住限流。前端 K线失败侧也做了 60 秒节流（NEW-8），避免持续失败的股票每 5 秒白打上游

**10. 腾讯美股 K线后缀 = 交易所标识，带错不报错只回 1 根**
- 后缀映射：`105 纳斯达克→.OQ`、`106 纽交所→.N`、`107 NYSE Arca/Amex→.AM`
- 实测：JPM/BAC 用 `.OQ` 只回当天 1 根 K线（接口仍返回 `code:0` 成功！），换 `.N` 回完整多日；SPY/VOO/DIA 要 `.AM`
- 这正是"返回残缺数据比报错更糟"的典型：`code:0` 不抛错就不会降级，前端只画一根，趋势全丢
- 解法：`fetchUSKline` 先用腾讯实时行情的 `p[2]`（形如 `SPY.AM`）确定后缀再拉 K线；查不到后缀或只回 1 根时显式抛错，降级 Yahoo
- 搜索路由的 `prefixMap` 必须包含 `107`，且未知市场（伦敦 155 等）要丢弃而不是默认套 `sh`——否则 ETF 会被加成 `shspy` 这种垃圾代码

**11. 各数据源估值字段覆盖不同（同屏卡片行数不一样的根因）**

| 源 | 市盈率 | 市净率 | 总市值 |
|---|---|---|---|
| 东财 `push2`（A股/港股主源） | `f162` | `f167` | `f116` |
| 新浪 `hq.sinajs`（A股备源） | 无，硬编码 `null` | 无 | 无 |
| 腾讯 `qt.gtimg` | A股 `p[52]`；港股/美股 `p[39]` | 仅 A股 `p[46]`；港股/美股该位置是英文名，无 | `p[44]`（亿元） |

- 所以降级时同屏卡片会不一致：A股落到新浪 → 估值三个字段全空；落到腾讯 → 三个都有；港股/美股在腾讯 → 缺市净率；ETF（SPY 等）本来就没有 PE
- **前端不要「字段为空就整行不渲染」**：那会让卡片高度/行数不同，看起来像"数据没加载全"。正确做法是始终渲染、缺失显示 `-`（和今开/昨收一致），再在降级角标的 tooltip 里按**实际字段**说明为什么是 `-`
- tooltip 里不能一律写"估值字段不可用"——腾讯降级时 A股估值字段齐全，那种写法是错的
